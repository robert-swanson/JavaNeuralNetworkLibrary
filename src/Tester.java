...
public class Tester {
		...
		do{
			...
			switch (input[0].toLowerCase()){
				...
				case "train":
					...
					bestNN.infiniteTrain(batchSize, learningRate);
					break;
				case "evaluate":
					double[][] stats = bestNN.testWithImages(images);
	...
	public static int testRealImage(String filename, boolean verbose) throws Exception {
		...
		int guess = bestNN.classify(image);
		System.out.printf("Guess: %d\n", guess);
		if(verbose) printCertainty(bestNN.getOutput());
		return guess;
	}
	...
	public static void testRandomImage() {
		...
		image.roughPrint();
		System.out.printf("Real: %d Guess: %d\n",image.label, bestNN.classify(image));
		printCertainty(bestNN.getOutput());
	}
